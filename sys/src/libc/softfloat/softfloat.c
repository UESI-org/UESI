...
*/
static void
normalizeFloat64Subnormal(bits32 aSig0,
                          bits32 aSig1,
                          int16 *zExpPtr,
                          bits32 *zSig0Ptr,
                          bits32 *zSig1Ptr)
	...
	if (0x7FD <= (bits16)zExp) {
		if ((0x7FD < zExp) ||
			...
			if ((roundingMode == float_round_to_zero) ||
				...
				return packFloat64(
				    zSign, 0x7FE, 0x000FFFFF, 0xFFFFFFFF);
			}
...
float32_to_int32(float32 a)
{
	...
	if (0 <= shiftCount) {
		...
		if (roundingMode == float_round_nearest_even) {
			...
			if (aSign) {
				z += (roundingMode == float_round_down) &
				     aSigExtra;
				z = -z;
			} else {
				z += (roundingMode == float_round_up) &
				     aSigExtra;
			}
		}
	}
...
float64_to_int32(float64 a)
{
	...
	if (roundingMode == float_round_nearest_even) {
		...
		if (aSign) {
			...
			z = absZ +
			    ((roundingMode == float_round_up) & aSigExtra);
		}
	}
	if ((aSign ^ (z < 0)) && z) {
invalid:
...
invalid:
	...
	while (0 < expDiff) {
		...
		shortShift96Left(
		    term0, term1, term2, 29, &term1, &term2, &allZero);
