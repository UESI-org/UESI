...
void kdebug_init(void) {
    ...
    if (serial_enabled) {
        ...
        serial_printf(debug_port, "Debug level: %s\n", level_names[debug_level]);
    }
}
...
void kdebug_set_level(int level) {
    if (level >= KDEBUG_LEVEL_NONE && level <= KDEBUG_LEVEL_TRACE) {
        debug_level = level;
        if (serial_enabled) {
            serial_printf(debug_port, "Debug level set to: %s\n", 
                         level_names[level]);
        }
    }
}
...
void kdebug_log(int level, const char *file, int line, 
    ...
    if (serial_enabled) {
        serial_printf(debug_port, "%s[%s:%d:%s] ", 
                     level_prefixes[level], file, line, func);
...
void kdebug_dump_registers(registers_t *regs) {
    ...
    if (serial_enabled) {
        serial_printf(debug_port, "\n=== Register Dump ===\n");
        serial_printf(debug_port, "RAX: 0x%016lx  RBX: 0x%016lx\n", 
                     regs->rax, regs->rbx);
        serial_printf(debug_port, "RCX: 0x%016lx  RDX: 0x%016lx\n", 
                     regs->rcx, regs->rdx);
        serial_printf(debug_port, "RSI: 0x%016lx  RDI: 0x%016lx\n", 
                     regs->rsi, regs->rdi);
        serial_printf(debug_port, "RBP: 0x%016lx  RSP: 0x%016lx\n", 
                     regs->rbp, regs->rsp);
        serial_printf(debug_port, "R8:  0x%016lx  R9:  0x%016lx\n", 
                     regs->r8, regs->r9);
        serial_printf(debug_port, "R10: 0x%016lx  R11: 0x%016lx\n", 
                     regs->r10, regs->r11);
        serial_printf(debug_port, "R12: 0x%016lx  R13: 0x%016lx\n", 
                     regs->r12, regs->r13);
        serial_printf(debug_port, "R14: 0x%016lx  R15: 0x%016lx\n", 
                     regs->r14, regs->r15);
        serial_printf(debug_port, "\nRIP: 0x%016lx  RFLAGS: 0x%016lx\n", 
                     regs->rip, regs->rflags);
        serial_printf(debug_port, "CS:  0x%04lx           SS:     0x%04lx\n", 
                     regs->cs, regs->ss);
        serial_printf(debug_port, "INT: %lu              ERR:    0x%lx\n", 
                     regs->int_no, regs->err_code);
    }
}
...
void kdebug_dump_control_registers(void) {
    ...
    if (serial_enabled) {
        serial_printf(debug_port, "\n=== Control Registers ===\n");
        serial_printf(debug_port, "CR0: 0x%016lx  CR2: 0x%016lx\n", cr0, cr2);
        serial_printf(debug_port, "CR3: 0x%016lx  CR4: 0x%016lx\n", cr3, cr4);
    }
}
...
void kdebug_backtrace_from(uint64_t rbp, uint64_t rip) {
    ...
    if (sym.name) {
        printf("#%d  0x%016lx in %s+0x%lx\n", depth, rip, sym.name, sym.offset);
        if (serial_enabled) {
            serial_printf(debug_port, "#%d  0x%016lx in %s+0x%lx\n", 
                         depth, rip, sym.name, sym.offset);
        }
    } else {
        printf("#%d  0x%016lx\n", depth, rip);
        if (serial_enabled) {
            serial_printf(debug_port, "#%d  0x%016lx\n", depth, rip);
        }
    }
    depth++;
    ...
    while (frame && depth < KDEBUG_MAX_STACK_DEPTH) {
        ...
        if (sym.name) {
            printf("#%d  0x%016lx in %s+0x%lx\n", 
                   depth, return_addr, sym.name, sym.offset);
            if (serial_enabled) {
                serial_printf(debug_port, "#%d  0x%016lx in %s+0x%lx\n", 
                             depth, return_addr, sym.name, sym.offset);
            }
...
void kdebug_hexdump(const void *data, size_t length) {
    ...
    for (size_t i = 0; i < length; i += 16) {
        ...
        if (serial_enabled) {
            serial_printf(debug_port, "%016lx: ", (uint64_t)data + i);
        }
        ...
        for (size_t j = 0; j < 16; j++) {
            if (i + j < length) {
                ...
                if (serial_enabled) {
                    serial_printf(debug_port, "%02x ", bytes[i + j]);
                }
            } else {
                printf("   ");
                if (serial_enabled) {
...
void kdebug_panic(const char *message, registers_t *regs) {
    ...
    for (int i = 0; i < MAX_BREAKPOINTS; i++) {
        if (!breakpoints[i].enabled) {
            ...
            *(uint8_t *)address = 0xCC;
            ...
            if (serial_enabled) {
                serial_printf(debug_port, "Breakpoint set at 0x%016lx\n", address);
            }
            return true;
        }
    }
    ...
    for (int i = 0; i < MAX_BREAKPOINTS; i++) {
        if (breakpoints[i].enabled && breakpoints[i].address == address) {
            ...
            *(uint8_t *)address = breakpoints[i].original_byte;
            ...
            if (serial_enabled) {
                serial_printf(debug_port, "Breakpoint cleared at 0x%016lx\n", address);
            }
            return true;
        }
    }
...
void kdebug_breakpoint_handler(registers_t *regs) {
    ...
    for (int i = 0; i < MAX_BREAKPOINTS; i++) {
        if (breakpoints[i].enabled && breakpoints[i].address == bp_address) {
            printf("\n=== Breakpoint Hit ===\n");
...
void kdebug_dump_page_info(uint64_t virtual_address) {
    ...
    if (serial_enabled) {
        serial_printf(debug_port, "\n=== Page Info for 0x%016lx ===\n", 
                     virtual_address);
        serial_printf(debug_port, "CR3 (Page Directory Base): 0x%016lx\n", cr3);
    }
...
void kdebug_dump_segments(void) {
    ...
    if (serial_enabled) {
        serial_printf(debug_port, "\n=== Segment Registers ===\n");
        serial_printf(debug_port, "CS: 0x%04x  DS: 0x%04x  ES: 0x%04x\n", 
                     cs, ds, es);
        serial_printf(debug_port, "FS: 0x%04x  GS: 0x%04x  SS: 0x%04x\n", 
                     fs, gs, ss);
    }
}