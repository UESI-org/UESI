...
#define KDEBUG_WARN(fmt, ...) KDEBUG_LOG(KDEBUG_LEVEL_WARN, fmt, ##__VA_ARGS__)

#define KDEBUG_INFO(fmt, ...) KDEBUG_LOG(KDEBUG_LEVEL_INFO, fmt, ##__VA_ARGS__)
...
void kdebug_assert_failed(const char *expr,
                          const char *file,
                          int line,
                          const char *func) __attribute__((noreturn));

void kdebug_assert_failed_msg(const char *expr,
                              const char *msg,
                              const char *file,
                              int line,
                              const char *func) __attribute__((noreturn));

void kdebug_log(int level,
                const char *file,
                int line,
                const char *func,
                const char *fmt,
