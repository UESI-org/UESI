...
void
tty_set_color(uint32_t fg, uint32_t bg)
{
	tty_t *tty = tty_get();
	tty->fg_color = fg;
	tty->bg_color = bg;
...
void
tty_get_color(uint32_t *fg, uint32_t *bg)
{
	tty_t *tty = tty_get();
...
void
tty_set_color_256(uint8_t fg, uint8_t bg)
{
	if (!palette_initialized) {
		tty_init_palette();
	}
...
uint32_t
tty_get_palette_color(uint8_t index)
{
	if (!palette_initialized) {
		tty_init_palette();
	}
...
void
tty_set_palette_color(uint8_t index, uint32_t rgb)
{
	if (!palette_initialized) {
		tty_init_palette();
	}
...
uint8_t
tty_rgb_to_palette(uint8_t r, uint8_t g, uint8_t b)
{
	if (!palette_initialized) {
		tty_init_palette();
	}
