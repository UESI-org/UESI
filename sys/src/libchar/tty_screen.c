...
void tty_scroll(void) {
    ...
    // opy entire rows at once using buffer pointer arithmetic
    for (uint64_t y = line_height; y < tty->height; y++) {
        uint64_t src_offset = y * pixels_per_row;
        uint64_t dst_offset = (y - line_height) * pixels_per_row;
        
        for (uint64_t x = 0; x < tty->width; x++) {
            tty->buffer[dst_offset + x] = tty->buffer[src_offset + x];
        }
    }
...
void tty_clear_to_end(void) {
    ...
    if (tty->cursor_y < tty->rows - 1) {
        ...
        tty_fill_rect(0, next_line_y, tty->width, 
                     tty->height - next_line_y, tty->bg_color);
    }
}
