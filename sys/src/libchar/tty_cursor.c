...
void
tty_set_cursor(uint64_t x, uint64_t y)
{
	tty_t *tty = tty_get();
	...
	if (x < tty->cols && y < tty->rows) {
		if (cursor_drawn && tty->cursor_visible) {
			...
			for (uint64_t cx = 0; cx < tty->char_width; cx++) {
				tty_putpixel(old_px + cx,
				             old_py + tty->char_height - 1,
				             tty->bg_color);
				tty_putpixel(old_px + cx,
				             old_py + tty->char_height - 2,
				             tty->bg_color);
			}
		}
...
void
tty_get_cursor(uint64_t *x, uint64_t *y)
{
	tty_t *tty = tty_get();
	...
	if (tty->cursor_visible && !visible) {
		...
		if (cursor_drawn) {
			...
			for (uint64_t x = 0; x < tty->char_width; x++) {
				tty_putpixel(px + x,
				             py + tty->char_height - 1,
				             tty->bg_color);
				tty_putpixel(px + x,
				             py + tty->char_height - 2,
				             tty->bg_color);
			}
			cursor_drawn = false;
		}
...
tty_draw_cursor(void)
{
	...
	if (cursor_drawn && (last_cursor_x != tty->cursor_x ||
	                     last_cursor_y != tty->cursor_y)) {
		uint64_t old_px = last_cursor_x * tty->char_width;
		uint64_t old_py = last_cursor_y * tty->char_height;

		for (uint64_t x = 0; x < tty->char_width; x++) {
			tty_putpixel(old_px + x,
			             old_py + tty->char_height - 1,
			             tty->bg_color);
			tty_putpixel(old_px + x,
			             old_py + tty->char_height - 2,
