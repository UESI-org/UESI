...
static void print_cpu_features(const cpu_info_t *info) {
    ...
    if (cpuid_has_avx512f()) serial_printf(DEBUG_PORT, "AVX512 ");
    if (cpuid_has_aes()) serial_printf(DEBUG_PORT, "AES ");
    if (cpuid_has_long_mode()) serial_printf(DEBUG_PORT, "x64 ");
    if (cpuid_has_nx()) serial_printf(DEBUG_PORT, "NX ");
    if (cpuid_has_1gb_pages()) serial_printf(DEBUG_PORT, "1GB-Pages ");
