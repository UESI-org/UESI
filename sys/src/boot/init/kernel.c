...
static void
initialize_system_components(void)
{
	...
	kdebug_init();
	isr_register_handler(3,
	                     kdebug_breakpoint_handler); // INT3 for breakpoints
...
run_tests(void)
{
	...
	if (modules && modules->module_count > 0) {
		...
		printf("Module: %s (%lu bytes)\n",
		       user_prog->path,
		       (unsigned long)user_prog->size);
		userland_load_and_run(
		    user_prog->address, user_prog->size, "test");
	} else {
...
kmain(void)
{
	...
	if (!boot_verify_protocol()) {
		panic("Limine base revision mismatch - incompatible bootloader "
		      "version");
	}
	...
	debug_framebuffer_info(framebuffer->width,
	                       framebuffer->height,
	                       framebuffer->pitch,
	                       framebuffer->bpp);
