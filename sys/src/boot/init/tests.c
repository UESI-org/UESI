...
test_ahci(void)
{
	...
	for (int i = 0; i < AHCI_MAX_PORTS; i++) {
		if (ctrl->ports[i].implemented &&
			...
			if (ahci_port_read(&ctrl->ports[i], 0, 1, buffer) ==
			    0) {
...
void
test_userland_load(const void *elf_data, size_t elf_size, const char *name)
{
...
bool
userland_load_and_run(const void *elf_data, size_t elf_size, const char *name)
{
	...
	for (size_t i = 0; i < stack_pages; i++) {
		...
		if (page_virt == NULL) {
			serial_printf(
			    DEBUG_PORT,
			    "ERROR: Failed to allocate physical page %lu\n",
			    (unsigned long)i);
		...
		if (!paging_map_range(
		        ps->ps_vmspace, virt_page, phys_page, 1, flags)) {
			serial_printf(
			    DEBUG_PORT,
