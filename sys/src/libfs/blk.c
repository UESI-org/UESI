...
static inline uint32_t blk_buffer_hash_func(dev_t dev, uint64_t block) {
...
int blk_write(blk_device_t *dev, uint64_t block, const void *buffer, size_t count) {
    if (dev == NULL || buffer == NULL || count == 0) {
        return -EINVAL;
    }
...
int blk_alloc_init(blk_allocator_t *alloc, uint32_t total_blocks, uint32_t first_data_block, uint32_t block_size) {
...
uint32_t blk_alloc_contiguous(blk_allocator_t *alloc, uint32_t count, uint32_t *blocks) {
    ...
    for (uint32_t start = 0; start <= alloc->ba_total_blocks - count; start++) {
        bool found = true;
...
int blk_free(blk_allocator_t *alloc, uint32_t block) {
...
int blk_free_contiguous(blk_allocator_t *alloc, uint32_t block, uint32_t count) {
...
bool blk_is_allocated(blk_allocator_t *alloc, uint32_t block) {
    if (alloc == NULL || block >= alloc->ba_total_blocks) {
        return false;
    }
...
void blk_alloc_stats(blk_allocator_t *alloc, uint32_t *total, 
    ...
    if (total) *total = alloc->ba_total_blocks;
    if (used) *used = alloc->ba_used_blocks;
    if (free) *free = alloc->ba_total_blocks - alloc->ba_used_blocks;
...
static int ramdisk_read(blk_device_t *dev, uint64_t block, 
...
static int ramdisk_write(blk_device_t *dev, uint64_t block, 
