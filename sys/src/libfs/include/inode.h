...
typedef struct superblock {
	...
	uint32_t
	    sb_reserved[10]; /* Reserved for future use (reduced from 12) */
} superblock_t;
...
void inode_alloc_stats(inode_alloc_t *alloc,
                       uint32_t *total,
                       uint32_t *used,
                       uint32_t *free);
...
int inode_alloc_block(inode_t *inode,
                      uint64_t file_block,
                      uint32_t *disk_block);
...
int superblock_init(superblock_t *sb,
                    uint32_t block_size,
...
inode_blocks_needed(uint64_t size, uint32_t block_size)
{
	return (size + block_size - 1) / block_size;
}

static inline bool
inode_is_dir(inode_t *inode)
{
	return (inode->i_disk.di_mode & VFS_IFMT) == VFS_IFDIR;
}

static inline bool
inode_is_reg(inode_t *inode)
{
	return (inode->i_disk.di_mode & VFS_IFMT) == VFS_IFREG;
}

static inline bool
inode_is_lnk(inode_t *inode)
{
	return (inode->i_disk.di_mode & VFS_IFMT) == VFS_IFLNK;
}

static inline uint64_t
inode_get_size(inode_t *inode)
{
	return inode->i_disk.di_size;
}

static inline void
inode_set_size(inode_t *inode, uint64_t size)
{
