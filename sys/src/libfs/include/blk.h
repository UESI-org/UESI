...
typedef struct blk_ops {
    int (*read)(struct blk_device *dev, uint64_t block, void *buffer, size_t count);
    int (*write)(struct blk_device *dev, uint64_t block, const void *buffer, size_t count);
...
uint32_t blk_free_multiple(blk_allocator_t *alloc, uint32_t *blocks, uint32_t count);
...
int blk_alloc_load(blk_allocator_t *alloc, struct vfs_mount *mount, uint32_t bitmap_block);
...
uint32_t blk_alloc_largest_free_region(blk_allocator_t *alloc, uint32_t *start_block);
...
static inline uint64_t blk_bytes_to_blocks(uint64_t bytes, uint32_t block_size) {
    return (bytes + block_size - 1) / block_size;
}
static inline uint64_t blk_blocks_to_bytes(uint64_t blocks, uint32_t block_size) {
    return blocks * block_size;
}
static inline bool blk_is_valid_block(blk_device_t *dev, uint64_t block) {
