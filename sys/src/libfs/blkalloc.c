...
#define BLKALLOC_DEBUG(fmt, ...) serial_printf(DEBUG_PORT, "[BLKALLOC] " fmt, ##__VA_ARGS__)

static inline bool bitmap_test_bit(uint8_t *bitmap, uint32_t bit) {
    uint32_t byte = bit / 8;
...
static inline uint32_t find_first_zero_bit_in_byte(uint8_t byte) {
...
int blk_alloc_init(blk_allocator_t *alloc, uint32_t total_blocks,
                   uint32_t first_data_block, uint32_t block_size) {
    ...
    BLKALLOC_DEBUG("Initialized: %u blocks, bitmap size %u bytes (%u blocks)\n",
                   total_blocks, bitmap_size, alloc->ba_bitmap_blocks);
...
void blk_alloc_destroy(blk_allocator_t *alloc) {
    ...
    if (alloc->ba_dirty && alloc->ba_mount != NULL) {
        BLKALLOC_DEBUG("Warning: destroying dirty allocator, syncing first\n");
...
uint32_t blk_alloc(blk_allocator_t *alloc) {
    ...
    if (alloc->ba_used_blocks >= alloc->ba_total_blocks) {
        ...
        BLKALLOC_DEBUG("Filesystem full: %u/%u blocks used\n",
                       alloc->ba_used_blocks, alloc->ba_total_blocks);
        return 0;
    }
    ...
    for (block = start; block < alloc->ba_total_blocks; block++) {
        if (!bitmap_test_bit(alloc->ba_bitmap, block)) {
            ...
            BLKALLOC_DEBUG("Allocated block %u (%u/%u used)\n",
                          block, alloc->ba_used_blocks, alloc->ba_total_blocks);
            return block;
        }
    }
    ...
    for (block = alloc->ba_first_data_block; block < start; block++) {
        if (!bitmap_test_bit(alloc->ba_bitmap, block)) {
            ...
            BLKALLOC_DEBUG("Allocated block %u (wrapped, %u/%u used)\n",
                          block, alloc->ba_used_blocks, alloc->ba_total_blocks);
            return block;
        }
    }
...
uint32_t blk_alloc_contiguous(blk_allocator_t *alloc, uint32_t count,
                               uint32_t *blocks) {
...
int blk_free(blk_allocator_t *alloc, uint32_t block_num) {
    ...
    BLKALLOC_DEBUG("Freed block %u (%u/%u used)\n",
                   block_num, alloc->ba_used_blocks, alloc->ba_total_blocks);
    return 0;
}

uint32_t blk_free_multiple(blk_allocator_t *alloc, uint32_t *blocks,
                            uint32_t count) {
...
int blk_alloc_sync(blk_allocator_t *alloc) {
    ...
    for (uint32_t i = 0; i < alloc->ba_bitmap_blocks; i++) {
        ...
        off_t seek_result = vfs_seek(dev_file, disk_offset, VFS_SEEK_SET);
        if (seek_result < 0) {
            BLKALLOC_DEBUG("Seek failed for block %u: %ld\n", block_num, seek_result);
            vfs_close(dev_file);
            return seek_result;
        }
        
        /* Write bitmap data */
        ssize_t written = vfs_write(dev_file, alloc->ba_bitmap + offset, size);
        if (written != (ssize_t)size) {
            BLKALLOC_DEBUG("Write failed for block %u: wrote %ld/%u bytes\n",
                          block_num, written, size);
            vfs_close(dev_file);
            return written < 0 ? written : -EIO;
        }
        
        BLKALLOC_DEBUG("Wrote bitmap block %u/%u (%u bytes at offset %lu)\n",
                      i + 1, alloc->ba_bitmap_blocks, size, disk_offset);
    }
    ...
    BLKALLOC_DEBUG("Synced bitmap to disk (%u blocks, %u bytes)\n",
                   alloc->ba_bitmap_blocks, alloc->ba_bitmap_size);
    return 0;
}

int blk_alloc_load(blk_allocator_t *alloc, struct vfs_mount *mount,
    ...
    for (uint32_t i = 0; i < alloc->ba_bitmap_blocks; i++) {
        ...
        /* Read bitmap data */
        ssize_t bytes_read = vfs_read(dev_file, alloc->ba_bitmap + offset, size);
        if (bytes_read != (ssize_t)size) {
            BLKALLOC_DEBUG("Read failed for block %u: read %ld/%u bytes\n",
                          block_num, bytes_read, size);
            vfs_close(dev_file);
            return bytes_read < 0 ? bytes_read : -EIO;
        }
        
        BLKALLOC_DEBUG("Read bitmap block %u/%u (%u bytes from offset %lu)\n",
                      i + 1, alloc->ba_bitmap_blocks, size, disk_offset);
    }
    ...
    for (uint32_t i = alloc->ba_first_data_block; i < alloc->ba_total_blocks; i++) {
        if (!bitmap_test_bit(alloc->ba_bitmap, i)) {
    ...
    BLKALLOC_DEBUG("Loaded bitmap from disk (block %u): %u/%u blocks used\n",
                   bitmap_block, alloc->ba_used_blocks, alloc->ba_total_blocks);
    return 0;
}
...
int blk_alloc_format(blk_allocator_t *alloc, uint32_t reserved_blocks) {
    ...
    BLKALLOC_DEBUG("Formatted: %u reserved blocks, %u data blocks\n",
                   reserved_blocks, alloc->ba_total_blocks - reserved_blocks);
    return 0;
}
...
void blk_alloc_debug_print(blk_allocator_t *alloc) {
    ...
    BLKALLOC_DEBUG("  Bitmap size:       %u bytes (%u blocks)\n",
                   alloc->ba_bitmap_size, alloc->ba_bitmap_blocks);
    ...
    uint32_t largest_size = blk_alloc_largest_free_region(alloc, &largest_start);
    BLKALLOC_DEBUG("  Largest free:      %u blocks at %u\n",
                   largest_size, largest_start);
...
int blk_alloc_validate(blk_allocator_t *alloc) {
    ...
    if (actual_used != alloc->ba_used_blocks) {
        BLKALLOC_DEBUG("CORRUPTION: used count mismatch (stored=%u, actual=%u)\n",
                       alloc->ba_used_blocks, actual_used);
        spinlock_release_irqrestore(&alloc->ba_lock, flags);
        return -EFAULT;
    }
    ...
    for (uint32_t i = 0; i < alloc->ba_first_data_block; i++) {
        if (!bitmap_test_bit(alloc->ba_bitmap, i)) {
            BLKALLOC_DEBUG("CORRUPTION: reserved block %u is marked free\n", i);
            spinlock_release_irqrestore(&alloc->ba_lock, flags);
            return -EFAULT;
        }
    }
