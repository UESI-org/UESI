    ...
    while (*current != NULL) {
        if (strcmp((*current)->mnt_point, mountpoint) == 0) {
            ...
            *current = mnt->mnt_next;
            ...
            if (mnt->mnt_point) kfree(mnt->mnt_point);
            if (mnt->mnt_device) kfree(mnt->mnt_device);
            kfree(mnt);
...
vnode_t *vfs_vnode_alloc(vfs_mount_t *mnt, ino_t ino) {
    ...
    /* Add to hash table */
    uint32_t hash = vnode_hash_func(0, ino); /* TODO: use proper device ID */
...
ssize_t vfs_write(vfs_file_t *file, const void *buf, size_t count) {
    ...
    uint64_t flags;
    spinlock_acquire_irqsave(&file->f_lock, &flags);
    off_t offset = (file->f_flags & VFS_O_APPEND) ? vnode->v_size : file->f_offset;
    spinlock_release_irqrestore(&file->f_lock, flags);
