...
#define TMPFS_DEBUG(fmt, ...) serial_printf(DEBUG_PORT, "[TMPFS] " fmt, ##__VA_ARGS__)
...
int tmpfs_mount(const char *device, const char *mountpoint,
    ...
    tmpfs_node_t *root = tmpfs_node_alloc(tm, TMPFS_DIR, S_IFDIR | TMPFS_DEFAULT_MODE);
    if (root == NULL) {
        kfree(tm);
        kfree(mount);
        return -ENOMEM;
...
ssize_t tmpfs_write(vnode_t *vnode, const void *buf, size_t count, off_t offset) {
    ...
    if (new_size > node->tn_data.file.capacity) {
        ...
        if (node->tn_data.file.data != NULL) {
            memcpy(new_data, node->tn_data.file.data, node->tn_size);
            kfree(node->tn_data.file.data);
        }
...
int tmpfs_truncate(vnode_t *vnode, off_t length) {
    ...
    if (length == 0) {
        ...
        if ((size_t)length > node->tn_data.file.capacity) {
            ...
            memset((char *)new_data + node->tn_size, 0, length - node->tn_size);
...
int tmpfs_dirent_add(tmpfs_node_t *dir, const char *name, tmpfs_node_t *node) {
    if (dir == NULL || name == NULL || node == NULL || dir->tn_type != TMPFS_DIR) {
        return -EINVAL;
    }
...
int tmpfs_link(vnode_t *dir, const char *name, vnode_t *target) {
    ...
    if (dir_node == NULL || target_node == NULL || dir_node->tn_type != TMPFS_DIR) {
        return -EINVAL;
    }
...
int tmpfs_symlink(vnode_t *dir, const char *name, const char *target) {
    ...
    tmpfs_node_t *symlink = tmpfs_node_alloc(tm, TMPFS_SYMLINK, S_IFLNK | 0777);
    if (symlink == NULL) {
        return -ENOMEM;
    }
...
static vnode_ops_t tmpfs_vnode_ops = {
    ...
    .sync = NULL,  /* No sync needed for tmpfs */
