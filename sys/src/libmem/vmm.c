...
static vmm_region_t *
vmm_create_region(uint64_t virt_start,
                  uint64_t virt_end,
...
static void
vmm_free_region(vmm_region_t *region)
{
	if (region != NULL) {
		pmm_free(region);
	}
...
static vmm_region_t *
vmm_find_region(vmm_address_space_t *space, uint64_t virt_addr)
{
	if (space == NULL) {
		return NULL;
	}
	...
	while (current != NULL) {
		if (virt_addr >= current->virt_start &&
		    virt_addr < current->virt_end) {
			return current;
		}
...
static bool
vmm_add_region(vmm_address_space_t *space, vmm_region_t *region)
{
...
static bool
vmm_remove_region(vmm_address_space_t *space, uint64_t virt_start)
{
...
vmm_address_space_t *
vmm_create_address_space(bool is_kernel)
{
...
void
vmm_destroy_address_space(vmm_address_space_t *space)
{
	if (space == NULL) {
		return;
	}
	...
	while (current != NULL) {
		...
		size_t num_pages =
		    (current->virt_end - current->virt_start) / PAGE_SIZE;
		paging_unmap_range(
		    space->page_dir, current->virt_start, num_pages);
...
vmm_address_space_t *
vmm_fork_address_space(vmm_address_space_t *parent)
{
	if (parent == NULL) {
		return NULL;
	}
	...
	while (current != NULL) {
		vmm_region_t *new_region =
		    vmm_create_region(current->virt_start,
		                      current->virt_end,
		                      current->type,
		                      current->flags);
		if (new_region == NULL) {
		...
		for (size_t i = 0; i < num_pages; i++) {
			...
			uint64_t phys =
			    mmu_get_physical_address(parent->page_dir, virt);

			if (phys != 0) {
				uint64_t cow_flags =
				    current->flags & ~PAGE_WRITE;

				mmu_map_page(
				    child->page_dir, virt, phys, cow_flags);

				paging_protect_range(
				    parent->page_dir, virt, 1, cow_flags);
			}
		}
...
void
vmm_switch_address_space(vmm_address_space_t *space)
{
...
vmm_address_space_t *
vmm_get_kernel_space(void)
{
	return kernel_space;
}

vmm_address_space_t *
vmm_get_current_space(void)
{
	return current_space;
}

void *
vmm_alloc(vmm_address_space_t *space, size_t size)
{
	...
	while (check != NULL) {
		if (!(virt_addr + size <= check->virt_start ||
		      virt_addr >= check->virt_end)) {
			return NULL;
		}
		check = check->next;
	}
	...
	for (size_t i = 0; i < num_pages; i++) {
		...
		if (phys_page == NULL) {
			for (size_t j = 0; j < i; j++) {
				...
				uint64_t phys = mmu_get_physical_address(
				    space->page_dir, virt);
				if (phys != 0) {
		...
		if (!mmu_map_page(space->page_dir,
	...
	if (stats.used_pages + num_pages >=
	    stats.used_pages) { // Overflow check
		stats.used_pages += num_pages;
	}

	vmm_region_t *region =
	    vmm_create_region(virt_addr,
	                      virt_addr + size,
	                      space == kernel_space ? VMM_REGION_KERNEL_HEAP
...
void *
vmm_alloc_at(vmm_address_space_t *space,
             uint64_t virt_addr,
             size_t size,
             uint64_t flags)
{
...
void
vmm_free(vmm_address_space_t *space, void *ptr, size_t size)
{
...
bool
vmm_map_region(vmm_address_space_t *space,
               uint64_t virt_start,
               size_t size,
               vmm_region_type_t type)
{
	...
	vmm_region_t *region =
	    vmm_create_region(virt_start, virt_start + size, type, flags);
	if (region == NULL) {
		return false;
	}
...
bool
vmm_unmap_region(vmm_address_space_t *space, uint64_t virt_start, size_t size)
{
	if (space == NULL || size == 0) {
		return false;
	}
...
bool
vmm_protect_region(vmm_address_space_t *space,
                   uint64_t virt_start,
                   size_t size,
                   uint64_t flags)
{
	...
	return paging_protect_range(
	    space->page_dir, virt_start, num_pages, flags);
}

void *
vmm_sbrk(vmm_address_space_t *space, intptr_t increment)
{
	if (space == NULL) {
		return (void *)-1;
	}
...
void
vmm_handle_page_fault(uint64_t fault_addr, uint64_t error_code)
{
	stats.page_faults++;
	...
	if (space == NULL) {
		serial_printf(DEBUG_PORT,
		              "Page fault in NULL address space at 0x%p\n",
		              (void *)fault_addr);
		return;
	}
	...
	if (region == NULL) {
		serial_printf(
		    DEBUG_PORT,
		    "Page fault at unmapped address 0x%p (error: 0x%p)\n",
		    (void *)fault_addr,
		    (void *)error_code);
		return;
	}
	...
	if (region->cow && (error_code & 0x2)) {
		...
		uint64_t old_phys =
		    mmu_get_physical_address(space->page_dir, page_addr);
		if (old_phys == 0) {
			serial_printf(DEBUG_PORT,
			              "COW fault but page not mapped at 0x%p\n",
			              (void *)fault_addr);
			return;
		}
		...
		if (new_page == NULL) {
			serial_printf(
			    DEBUG_PORT,
			    "Failed to allocate page for COW at 0x%p\n",
			    (void *)fault_addr);
			return;
		}
		...
		if (!mmu_map_page(
		        space->page_dir, page_addr, new_phys, new_flags)) {
			serial_printf(DEBUG_PORT,
			              "Failed to remap COW page at 0x%p\n",
			              (void *)fault_addr);
			pmm_free(new_page);
			return;
		}
		size_t region_pages =
		    (region->virt_end - region->virt_start) / PAGE_SIZE;

		vmm_flush_tlb(page_addr);

		serial_printf(DEBUG_PORT,
		              "COW page fault resolved at 0x%p\n",
		              (void *)fault_addr);
	} else {
		serial_printf(DEBUG_PORT,
		              "Unhandled page fault at 0x%p (error: 0x%p)\n",
		              (void *)fault_addr,
		              (void *)error_code);
	}
}
...
void
vmm_flush_tlb(uint64_t virt_addr)
{
...
void
vmm_dump_regions(vmm_address_space_t *space)
{
	if (space == NULL) {
	...
	while (current != NULL) {
		serial_printf(DEBUG_PORT,
		              "  [%d] 0x%p - 0x%p (type=%d, cow=%d)\n",
		              count++,
		              (void *)current->virt_start,
		              (void *)current->virt_end,
		              current->type,
		              current->cow);
		current = current->next;
	}
