...
static void slab_mark_free(slab_t *slab, int index) {
...
static bool find_allocation(void *ptr, cache_t **out_cache, slab_t **out_slab, int *out_index) {
    if (ptr == NULL) {
        return false;
    }
...
static bool is_object_free(slab_t *slab, int index) {
    size_t bitmap_index = index / 64;
    size_t bit_index = index % 64;
    return (slab->free_bitmap[bitmap_index] & (1ULL << bit_index)) != 0;
}

static void cache_free(cache_t *cache, slab_t *slab, int obj_index) {
    if (is_object_free(slab, obj_index)) {
        serial_printf(DEBUG_PORT, "[kfree] WARNING: Double-free detected in cache %s\n",
                     cache->name);
        return;
    }
