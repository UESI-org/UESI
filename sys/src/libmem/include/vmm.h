...
void *vmm_alloc_at(vmm_address_space_t *space,
                   uint64_t virt_addr,
                   size_t size,
                   uint64_t flags);
void vmm_free(vmm_address_space_t *space, void *ptr, size_t size);
bool vmm_map_region(vmm_address_space_t *space,
                    uint64_t virt_start,
                    size_t size,
                    vmm_region_type_t type);
bool vmm_unmap_region(vmm_address_space_t *space,
                      uint64_t virt_start,
                      size_t size);
bool vmm_protect_region(vmm_address_space_t *space,
                        uint64_t virt_start,
                        size_t size,
                        uint64_t flags);
