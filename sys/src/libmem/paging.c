...
bool
paging_map_range(page_directory_t *pd,
                 uint64_t virt_base,
                 uint64_t phys_base,
                 size_t num_pages,
                 uint64_t flags)
...
bool
paging_unmap_range(page_directory_t *pd, uint64_t virt_base, size_t num_pages)
{
	if (pd == NULL) {
		return false;
	}
...
bool
paging_map_region(page_directory_t *pd, memory_region_t *region)
{
	...
	if (!paging_map_range(
	        pd, region->base, phys_addr, num_pages, region->flags)) {
...
bool
paging_identity_map(page_directory_t *pd,
                    uint64_t phys_base,
                    size_t num_pages,
                    uint64_t flags)
{
...
bool
paging_is_range_mapped(page_directory_t *pd,
                       uint64_t virt_base,
                       size_t num_pages)
{
	if (pd == NULL) {
...
page_directory_t *
paging_create_kernel_address_space(void)
{
	return mmu_create_address_space();
}

page_directory_t *
paging_create_user_address_space(void)
{
...
page_directory_t *
paging_clone_address_space(page_directory_t *src)
{
	...
	for (int pml4_idx = 0; pml4_idx < 256; pml4_idx++) {
		...
		for (int pdpt_idx = 0; pdpt_idx < 512; pdpt_idx++) {
			...
			if (pdpt[pdpt_idx] & PAGE_HUGE)
				continue; // Skip huge pages for now
			...
			for (int pd_idx = 0; pd_idx < 512; pd_idx++) {
				...
				for (int pt_idx = 0; pt_idx < 512; pt_idx++) {
					...
					if (new_page == NULL) {
						mmu_destroy_address_space(
						    new_pd);
						return NULL;
					}

					void *src_page_virt =
					    mmu_phys_to_virt(src_page_phys);
					memcpy(
					    new_page, src_page_virt, PAGE_SIZE);

					uint64_t new_phys =
					    mmu_virt_to_phys(new_page);
...
void
paging_switch_directory(page_directory_t *pd)
{
	if (pd == NULL) {
		return;
	}
...
page_directory_t *
paging_get_kernel_directory(void)
{
	return kernel_directory;
}

page_directory_t *
paging_get_current_directory(void)
{
	return mmu_get_current_address_space();
}

uint64_t
paging_get_flags_for_region_type(region_type_t type)
{
	switch (type) {
...
bool
paging_protect_range(page_directory_t *pd,
                     uint64_t virt_base,
                     size_t num_pages,
                     uint64_t flags)
{
...
void
paging_free_user_pages(page_directory_t *pd)
{
	...
	serial_printf(
	    DEBUG_PORT, "Freeing user pages for page directory %p\n", pd);
	...
	serial_printf(DEBUG_PORT,
	              "Freed %llu user pages (%llu KB)\n",
	              pages_freed,
	              pages_freed * 4);
	serial_printf(DEBUG_PORT,
	              "Freed %llu page table structures (%llu KB)\n",
	              tables_freed,
	              tables_freed * 4);
}

void
paging_dump_address_space(page_directory_t *pd)
{
	if (pd == NULL || pd->pml4 == NULL) {
	...
	for (uint64_t pml4_idx = 0; pml4_idx < 512; pml4_idx++) {
		...
		for (uint64_t pdpt_idx = 0; pdpt_idx < 512; pdpt_idx++) {
			...
			for (uint64_t pd_idx = 0; pd_idx < 512; pd_idx++) {
				...
				for (uint64_t pt_idx = 0; pt_idx < 512;
				     pt_idx++) {
					if (!(pt[pt_idx] & PAGE_PRESENT))
						continue;
	...
	serial_printf(
	    DEBUG_PORT, "Total mapped memory: %llu KB\n", (mapped_pages * 4));
	serial_printf(DEBUG_PORT, "=========================\n\n");
}