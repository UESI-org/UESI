...
static inline void *kernel_mmap(void *addr, size_t len, int prot, int flags, int fd, off_t off) {
...
static inline int kernel_munmap(void *addr, size_t len) {
...
static void *large_alloc(size_t size) {
    ...
    void *mem = USER_MMAP(NULL, total_size, PROT_READ | PROT_WRITE,
                          MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
