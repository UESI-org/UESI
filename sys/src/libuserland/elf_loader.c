	...
	for (int i = 0; i < ehdr->e_phnum; i++) {
		...
		if (phdr->p_type == PT_LOAD) {
			...
			if (phdr->p_offset + phdr->p_filesz > size) {
				printf_("ELF: Segment %d data outside file "
				        "bounds\n",
				        i);
				return false;
			}
			...
			if (phdr->p_align > 0 &&
				...
				printf_("ELF: Segment %d has invalid alignment "
				        "0x%lx\n",
				        i,
				        phdr->p_align);
				return false;
			...
			if (phdr->p_vaddr + phdr->p_memsz > USER_SPACE_MAX) {
				printf_("ELF: Segment %d extends into kernel "
				        "space\n",
				        i);
				return false;
			}
...
write_to_virtual_memory(struct process *ps,
                        ...
                        bool zero_fill)
{
	extern uint64_t mmu_get_physical_address(page_directory_t * pd,
	                                         uint64_t virt);
         ...
         uint64_t *entry_point)
{
	if (!ps || !elf_data || !entry_point) {
		printf_(
		    "ELF: NULL parameter - ps=%p elf_data=%p entry_point=%p\n",
		    ps,
		    elf_data,
		    entry_point);
		return false;
	...
	for (int i = 0; i < ehdr->e_phnum; i++) {
		...
		if (phdr->p_type == PT_LOAD) {
			...
			uint64_t virt_end =
			    (phdr->p_vaddr + phdr->p_memsz + 0xFFF) & ~0xFFFULL;
	...
	for (int i = 0; i < ehdr->e_phnum; i++) {
		...
		printf_("  Segment %d: vaddr=0x%lx filesz=0x%lx memsz=0x%lx "
		        "flags=0x%x\n",
		        i,
		        phdr->p_vaddr,
		        phdr->p_filesz,
		...
		for (size_t j = 0; j < num_pages; j++) {
			...
			if (page_virt == NULL) {
				printf_(
				    "ELF: Failed to allocate physical page\n");
				/* TODO: Cleanup allocated pages */
				return false;
			}

			/* Convert to physical address by subtracting HHDM
			 * offset */
		...
		if (phdr->p_filesz > 0) {
			...
			serial_printf(
			    DEBUG_PORT,
			    "  Copying %lu bytes starting at vaddr=0x%lx\n",
		...
		if (phdr->p_memsz > phdr->p_filesz) {
			...
			serial_printf(
			    DEBUG_PORT,
			    "  Zeroing %lu bytes starting at vaddr=0x%lx\n",
			    (unsigned long)zero_size,
			    zero_start);

			if (!write_to_virtual_memory(
			        ps, zero_start, NULL, zero_size, true)) {
