 ...
 *
 * The formula: load_new = load_old * exp(-interval/period) + active * (1 -
 * exp(-interval/period))
 *
...
static void
get_load_averages(uint64_t loads[3])
{
...
static uint32_t
calculate_mem_usage_percent(uint64_t total, uint64_t free)
{
	if (total == 0)
		return 0;
...
int
kern_sysinfo(struct sysinfo *info)
{
	if (info == NULL) {
		return -1;
	}
