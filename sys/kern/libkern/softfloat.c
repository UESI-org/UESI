...
*/
static int64
roundAndPackInt64(flag zSign, bits64 absZ0, bits64 absZ1)
{
	int8 roundingMode;
	...
	if (!roundNearestEven) {
		if (roundingMode == float_round_to_zero) {
			...
			if (zSign) {
				increment =
				    (roundingMode == float_round_down) && absZ1;
			} else {
				increment =
				    (roundingMode == float_round_up) && absZ1;
			}
		}
	}
	if (increment) {
...
*/
static int64
roundAndPackInt64NoOverflow(flag zSign, bits64 absZ0, bits64 absZ1)
{
	int8 roundingMode;
	flag roundNearestEven, increment;
	int64 z;
...
normalizeRoundAndPackFloat32(flag zSign, int16 zExp, bits32 zSig)
{
	...
	return roundAndPackFloat32(
	    zSign, zExp - shiftCount, zSig << shiftCount);
}
...
{
	return FLOAT64_MANGLE((((bits64)zSign) << 63) + (((bits64)zExp) << 52) +
	                      zSig);
}
...
*/
INLINE floatx80
packFloatx80(flag zSign, int32 zExp, bits64 zSig)
{
	floatx80 z;
...
precision80:
	...
	if (!roundNearestEven) {
		if (roundingMode == float_round_to_zero) {
			...
			if (zSign) {
				increment =
				    (roundingMode == float_round_down) && zSig1;
			} else {
				increment =
				    (roundingMode == float_round_up) && zSig1;
			}
		}
	}
	if (0x7FFD <= (bits32)(zExp - 1)) {
...
overflow:
			...
			return packFloatx80(
			    zSign, 0x7FFF, LIT64(0x8000000000000000));
		}
		if (zExp <= 0) {
			isTiny = (float_detect_tininess ==
			          float_tininess_before_rounding) ||
			...
			shift64ExtraRightJamming(
			    zSig0, zSig1, 1 - zExp, &zSig0, &zSig1);
			zExp = 0;
			if (isTiny && zSig1)
				float_raise(float_flag_underflow);
			if (zSig1)
...
*/
static void
normalizeFloat128Subnormal(bits64 aSig0,
                           bits64 aSig1,
                           int32 *zExpPtr,
                           bits64 *zSig0Ptr,
                           bits64 *zSig1Ptr)
    ...
    flag zSign, int32 zExp, bits64 zSig0, bits64 zSig1, bits64 zSig2)
{
	...
	if (!roundNearestEven) {
		if (roundingMode == float_round_to_zero) {
			...
			if (zSign) {
				increment =
				    (roundingMode == float_round_down) && zSig2;
			} else {
				increment =
				    (roundingMode == float_round_up) && zSig2;
			}
		}
	}
...
int64_to_float32(int64 a)
{
	...
	if (0 <= shiftCount) {
		return packFloat32(
		    zSign, 0x95 - shiftCount, absA << shiftCount);
	} else {
		shiftCount += 7;
		if (shiftCount < 0) {
...
float64_to_int64(float64 a)
{
	...
	if (shiftCount <= 0) {
		...
		shift64ExtraRightJamming(
		    aSig, 0, shiftCount, &aSig, &aSigExtra);
	}
...
*/
int64
float64_to_int64_no_overflow(float64 a)
{
	flag aSign;
	int16 aExp, shiftCount;
	bits64 aSig, aSigExtra;
...
float64_round_to_int(float64 a)
{
	...
	if (aExp < 0x3FF) {
		...
		case float_round_up:
			return aSign ? LIT64(0x8000000000000000)
...
float64_eq(float64 a, float64 b)
{
	...
	return (a == b) || ((bits64)((FLOAT64_DEMANGLE(a) | FLOAT64_DEMANGLE(b))
	                             << 1) == 0);
}
...
float64_le(float64 a, float64 b)
{
	...
	if (aSign != bSign)
		return aSign ||
		       ((bits64)((FLOAT64_DEMANGLE(a) | FLOAT64_DEMANGLE(b))
		                 << 1) == 0);
	return (a == b) ||
	...
	if (aExp < 0x3FFF) {
		...
		case float_round_nearest_even:
			if ((aExp == 0x3FFE) &&
				...
				return packFloatx80(
				    aSign, 0x3FFF, LIT64(0x8000000000000000));
			}
			break;
		case float_round_down:
			return aSign ? packFloatx80(
			                   1, 0x3FFF, LIT64(0x8000000000000000))
			             : packFloatx80(0, 0, 0);
		case float_round_up:
...
*/
static floatx80
addFloatx80Sigs(floatx80 a, floatx80 b, flag zSign)
{
...
*/
static floatx80
subFloatx80Sigs(floatx80 a, floatx80 b, flag zSign)
{
	int32 aExp, bExp, zExp;
	bits64 aSig, bSig, zSig0, zSig1;
	int32 expDiff;
...
bExpBigger:
	if (bExp == 0x7FFF) {
		if ((bits64)(bSig << 1))
			return propagateFloatx80NaN(a, b);
		return packFloatx80(
		    zSign ^ 1, 0x7FFF, LIT64(0x8000000000000000));
	}
	if (aExp == 0)
		++expDiff;
...
*/
floatx80
floatx80_add(floatx80 a, floatx80 b)
{
	flag aSign, bSign;
...
*/
floatx80
floatx80_sub(floatx80 a, floatx80 b)
{
	flag aSign, bSign;
...
*/
floatx80
floatx80_mul(floatx80 a, floatx80 b)
{
	flag aSign, bSign, zSign;
	int32 aExp, bExp, zExp;
	bits64 aSig, bSig, zSig0, zSig1;
...
*/
floatx80
floatx80_div(floatx80 a, floatx80 b)
{
	flag aSign, bSign, zSign;
	int32 aExp, bExp, zExp;
	bits64 aSig, bSig, zSig0, zSig1;
...
*/
floatx80
floatx80_rem(floatx80 a, floatx80 b)
{
	flag aSign, bSign, zSign;
...
floatx80_le(floatx80 a, floatx80 b)
{
	...
	bSign = extractFloatx80Sign(b);
	if (aSign != bSign) {
		return aSign || ((((bits16)((a.high | b.high) << 1)) | a.low |
		                  b.low) == 0);
	}
...
*/
flag
floatx80_eq_signaling(floatx80 a, floatx80 b)
{
	if (((extractFloatx80Exp(a) == 0x7FFF) &&
	     (bits64)(extractFloatx80Frac(a) << 1)) ||
	    ((extractFloatx80Exp(b) == 0x7FFF) &&
...
*/
flag
floatx80_le_quiet(floatx80 a, floatx80 b)
{
	flag aSign, bSign;
...
*/
flag
floatx80_lt_quiet(floatx80 a, floatx80 b)
{
	flag aSign, bSign;
...
float128_to_int64(float128 a)
{
	...
	if (shiftCount <= 0) {
		...
		shift64ExtraRightJamming(
		    aSig0, aSig1, shiftCount, &aSig0, &aSig1);
	}
...
float128_round_to_int(float128 a)
{
	...
	if (0x402F <= aExp) {
		if (0x406F <= aExp) {
			if ((aExp == 0x7FFF) && (extractFloat128Frac0(a) |
...
*/
static float128
addFloat128Sigs(float128 a, float128 b, flag zSign)
{
	int32 aExp, bExp, zExp;
...
*/
static float128
subFloat128Sigs(float128 a, float128 b, flag zSign)
{
	int32 aExp, bExp, zExp;
...
*/
float128
float128_add(float128 a, float128 b)
{
	flag aSign, bSign;
...
*/
float128
float128_sub(float128 a, float128 b)
{
	flag aSign, bSign;
...
*/
float128
float128_mul(float128 a, float128 b)
{
	flag aSign, bSign, zSign;
	int32 aExp, bExp, zExp;
...
*/
float128
float128_div(float128 a, float128 b)
{
	flag aSign, bSign, zSign;
	int32 aExp, bExp, zExp;
	bits64 aSig0, aSig1, bSig0, bSig1, zSig0, zSig1, zSig2;
...
*/
float128
float128_rem(float128 a, float128 b)
{
	flag aSign, bSign, zSign;
	int32 aExp, bExp, expDiff;
...
invalid:
	...
	if (expDiff < -1)
		return a;
	shortShift128Left(aSig0 | LIT64(0x0001000000000000),
	                  aSig1,
	                  15 - (expDiff < 0),
	                  &aSig0,
	                  &aSig1);
	...
	while (0 < expDiff) {
		...
		shortShift192Left(
		    term0, term1, term2, 61, &term1, &term2, &allZero);
	...
	return normalizeRoundAndPackFloat128(
	    aSign ^ zSign, bExp - 4, aSig0, aSig1);
}
...
float128_le(float128 a, float128 b)
{
	...
	bSign = extractFloat128Sign(b);
	if (aSign != bSign) {
		return aSign || ((((bits64)((a.high | b.high) << 1)) | a.low |
		                  b.low) == 0);
	}
	return aSign ? le128(b.high, b.low, a.high, a.low)
	             : le128(a.high, a.low, b.high, b.low);
...
*/
flag
float128_eq_signaling(float128 a, float128 b)
{
	if (((extractFloat128Exp(a) == 0x7FFF) &&
	     (extractFloat128Frac0(a) | extractFloat128Frac1(a))) ||
	    ((extractFloat128Exp(b) == 0x7FFF) &&
...
*/
flag
float128_le_quiet(float128 a, float128 b)
{
	flag aSign, bSign;
...
*/
flag
float128_lt_quiet(float128 a, float128 b)
{
	flag aSign, bSign;
