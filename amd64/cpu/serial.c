...
bool serial_init_custom(uint16_t port, uint16_t divisor, uint8_t data_bits,
    ...
    if (inb(port + SERIAL_DATA_REG) != 0xAE) return false;
...
void serial_write_string(uint16_t port, const char *str) {
    if (!str) return;
    while (*str) serial_write_byte(port, *str++);
}

void serial_write(uint16_t port, const uint8_t *buffer, size_t size) {
    if (!buffer) return;
    for (size_t i = 0; i < size; i++) serial_write_byte(port, buffer[i]);
}
...
static void print_num(uint16_t port, unsigned long long num, int base, int width, char pad, bool uppercase) {
    char buf[32];
    int i = 0;
    ...
    if (num == 0) buf[i++] = '0';
        ...
        while (num > 0) {
            ...
            else buf[i++] = (uppercase ? 'A' : 'a') + (digit - 10);
            num /= base;
        }
    }
    ...
    while (i-- > 0) serial_write_byte(port, buf[i]);
}

static void print_signed(uint16_t port, long long num, int width, char pad) {
    if (num < 0) {
    ...
    while (*fmt) {
        if (*fmt == '%' && *(fmt + 1)) {
            ...
            switch (*fmt) {
                case 'd':
                case 'i':
                    if (is_long_long) print_signed(port, va_arg(args, long long), width, pad);
                    else print_signed(port, va_arg(args, int), width, pad);
                    break;
                case 'u':
                    if (is_long_long) print_num(port, va_arg(args, unsigned long long), 10, width, pad, false);
                    else print_num(port, va_arg(args, unsigned int), 10, width, pad, false);
                    break;
                case 'x':
                    if (is_long_long) print_num(port, va_arg(args, unsigned long long), 16, width, pad, false);
                    else print_num(port, va_arg(args, unsigned int), 16, width, pad, false);
                    break;
                case 'X':
                    if (is_long_long) print_num(port, va_arg(args, unsigned long long), 16, width, pad, true);
                    else print_num(port, va_arg(args, unsigned int), 16, width, pad, true);
                    break;
                case 'p':
                    serial_write_string(port, "0x");
                    print_num(port, (unsigned long long)va_arg(args, void*), 16, width, pad, false);
                    break;
                case 's': {
                ...
                case 'c':
                    serial_write_byte(port, (char)va_arg(args, int));
                    break;
                case '%':
                    serial_write_byte(port, '%');
                    break;
