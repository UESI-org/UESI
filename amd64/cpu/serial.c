...
static void print_num(uint16_t port, unsigned long long num, int base, 
                     int width, char pad, bool uppercase) {
    char buf[32];
    int i = 0;
    ...
    while (*fmt) {
        if (*fmt == '%' && *(fmt + 1)) {
            ...
            switch (*fmt) {
                ...
                case 'i':
                    if (is_long_long) {
                        print_signed(port, va_arg(args, long long), width, pad);
                    } else {
                        print_signed(port, va_arg(args, int), width, pad);
                    }
                    break;
                case 'u':
                    if (is_long_long) {
                        print_num(port, va_arg(args, unsigned long long), 
                                 10, width, pad, false);
                    } else {
                        print_num(port, va_arg(args, unsigned int), 
                                 10, width, pad, false);
                    }
                    break;
                case 'x':
                ...
                case 'p':
                    ...
                    print_num(port, (unsigned long long)va_arg(args, void*), 
                             16, width, pad, false);
                    break;
                case 's': {
