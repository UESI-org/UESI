...
static void
serial_write_direct(uint16_t port, const char *str)
{
	while (*str) {
		/* Wait for transmit buffer to be empty */
		while (!(inb(port + SERIAL_LINE_STATUS_REG) &
...
dump_registers(registers_t *regs)
{
	...
	serial_printf(PANIC_SERIAL_PORT,
...
static void
dump_stack_trace(uint64_t rbp, uint64_t rip)
{
	int frame = 0;
	...
	for (frame = 1; frame < PANIC_MAX_STACK_FRAMES; frame++) {
		if (!is_valid_address((uint64_t)frame_ptr))
			break;
		if (frame_ptr == NULL || (uint64_t)frame_ptr < 0x1000)
			break;
		if ((uint64_t)frame_ptr == rbp)
			break;

		uint64_t ret_addr = frame_ptr[1];
		uint64_t next_rbp = frame_ptr[0];

		if (ret_addr == 0 || ret_addr < 0x1000)
			break;

		tty_printf("#%d:  0x%016llx\n", frame, ret_addr);
		serial_printf(
		    PANIC_SERIAL_PORT, "#%d:  0x%016llx\r\n", frame, ret_addr);

		if (!is_valid_address(next_rbp) ||
		    next_rbp <= (uint64_t)frame_ptr)
			break;
...
__attribute__((noreturn)) void
panic_registers(const char *message, registers_t *regs, panic_code_t code)
{
	/* Prevent re-entrant panics */
	if (in_panic) {
		halt_cpu();
...
__attribute__((noreturn)) void
panic_assert(const char *expr, const char *file, int line, const char *func)
{
	/* Prevent re-entrant panics */
	if (in_panic) {
		halt_cpu();
...
bool
panic_in_progress(void)
{
	return in_panic;
}

const char *
panic_code_name(panic_code_t code)
{
	switch (code) {
	case PANIC_GENERAL:
