...
uint32_t
pit_set_frequency(uint32_t frequency)
{
	...
	// Send command byte: Channel 0, Access mode lo/hi byte, Mode 2 (rate
	// generator), Binary
...
void
pit_set_count(pit_channel_t channel, uint16_t divisor, pit_mode_t mode)
{
	uint8_t command;
	uint16_t port;
	...
	command = ((channel & 0x3) << 6) | PIT_ACCESS_LOHI |
	          ((mode & 0x7) << 1) | PIT_BINARY;
...
uint16_t
pit_read_count(pit_channel_t channel)
{
	uint8_t low, high;
	uint16_t port;
	uint8_t command;
...
void
pit_oneshot(uint32_t microseconds)
{
	uint16_t divisor;
...
uint16_t
pit_calculate_divisor(uint32_t frequency)
{
	uint32_t divisor;
...
void
pit_delay(uint32_t milliseconds)
{
