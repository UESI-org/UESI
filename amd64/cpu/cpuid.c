 ...
 */
void
cpuid_exec(uint32_t leaf, uint32_t subleaf, cpuid_regs_t *regs)
{
	if (!regs) {
		return;
	}
 ...
 */
bool
cpuid_has_feature(uint32_t feature_flag)
{
 ...
 */
bool
cpuid_has_ext_feature(uint32_t feature_flag)
{
	const cpu_info_t *info = &g_cpu_info;
 ...
 */
bool
cpuid_has_ext7_feature(uint32_t feature_flag)
{
	const cpu_info_t *info = &g_cpu_info;
 ...
 */
uint32_t
cpuid_get_cache_line_size(void)
{
	if (!g_cpu_info_initialized) {
		return 0;
	}
...
cpuid_init(cpu_info_t *info)
{
	...
	*((uint32_t *)(target_info->vendor + 8)) = regs.ecx;
	...
	/* Detect vendor */
	target_info->is_intel =
	    str_equal(target_info->vendor, CPU_VENDOR_INTEL);
	...
	if (target_info->max_basic_leaf >= CPUID_FEATURES) {
		...
		if (target_info->family == 0xF) {
			target_info->display_family =
			    target_info->family + target_info->ext_family;
		}
		if (target_info->family == 0x6 || target_info->family == 0xF) {
			target_info->display_model =
			    target_info->model + (target_info->ext_model << 4);
		}
		...
		target_info->cache_line_size =
		    ((regs.ebx >> 8) & 0xFF) * 8; /* In bytes */
 ...
 */
void
cpuid_print_info(const cpu_info_t *info)
{
...
void
cpuid_print_features(const cpu_info_t *info)
{
	if (!info || !info->cpuid_supported) {
		return;
	}
	...
	if (cpuid_has_sse3())
		printf("SSE3 ");
	if (cpuid_has_pclmulqdq())
		printf("PCLMUL ");
	if (cpuid_has_monitor())
		printf("MONITOR ");
	if (cpuid_has_ssse3())
		printf("SSSE3 ");
	if (cpuid_has_fma())
	...
	if (cpuid_has_adx())
		printf("ADX ");
	if (cpuid_has_clflushopt())
		printf("CLFLUSHOPT ");
	if (cpuid_has_clwb())
		printf("CLWB ");
	if (cpuid_has_processor_trace())
		printf("PT ");
	if (cpuid_has_sha())
		printf("SHA ");
	printf("\n");
	...
	if (has_avx512) {
		printf("  AVX-512: F ");
		if (cpuid_has_avx512dq())
			printf("DQ ");
		if (cpuid_has_avx512_ifma())
			printf("IFMA ");
		if (cpuid_has_avx512bw())
			printf("BW ");
		if (cpuid_has_avx512vl())
			printf("VL ");
		printf("\n");
	}
	...
	if (cpuid_has_nx())
		printf("NX ");
	if (cpuid_has_syscall())
		printf("SYSCALL ");
	if (cpuid_has_rdtscp())
		printf("RDTSCP ");
	if (cpuid_has_1gb_pages())
		printf("1GB_PAGES ");
	if (cpuid_has_lahf_lm())
		printf("LAHF_LM ");
	if (cpuid_has_lzcnt())
		printf("LZCNT ");
	if (cpuid_has_prefetchw())
		printf("PREFETCHW ");
	printf("\n");
	...
	if (cpuid_has_stibp())
		printf("STIBP ");
	if (cpuid_has_l1d_flush())
		printf("L1D_FLUSH ");
	if (cpuid_has_arch_capabilities())
		printf("ARCH_CAP ");
	if (cpuid_has_ssbd())
		printf("SSBD ");
