...
void
timer_set_frequency(uint32_t frequency_hz)
{
	...
	uint8_t command =
	    PIT_CHANNEL_0 | PIT_ACCESS_LOHI | PIT_MODE_2 | PIT_BINARY;
...
timer_init(uint32_t frequency_hz)
{
	...
	idt_set_gate(32 + TIMER_IRQ,
	             (uint64_t)timer_interrupt_handler,
...
uint64_t
timer_get_uptime_sec(void)
{
	return timer_ticks / timer_frequency;
}

void
timer_delay_ticks(uint64_t ticks)
{
...
static timer_callback_node_t *
alloc_callback_node(void)
{
...
timer_register_oneshot(timer_callback_t callback, void *data, uint32_t delay_ms)
{
	if (callback == NULL) {
		return NULL;
	}
...
timer_callback_node_t *
timer_register_periodic(timer_callback_t callback,
                        void *data,
...
void
timer_cancel_callback(timer_callback_node_t *node)
{
	if (node != NULL) {
		node->active = 0;
	}
