...
pci_config_address(uint8_t bus,
                   uint8_t device,
                   uint8_t function,
                   uint8_t offset)
{
...
void
pci_config_write32(uint8_t bus,
                   uint8_t device,
                   uint8_t function,
                   uint8_t offset,
                   uint32_t value)
...
void
pci_config_write16(uint8_t bus,
                   uint8_t device,
                   uint8_t function,
                   uint8_t offset,
                   uint16_t value)
...
{
	uint16_t vendor_id =
	    pci_config_read16(bus, device, function, PCI_CONFIG_VENDOR_ID);
...
{
	uint8_t header_type =
	    pci_config_read8(bus, device, 0, PCI_CONFIG_HEADER_TYPE);
             ...
             pci_bar_t *bar)
{
	...
	if (bar_value & PCI_BAR_IO) {
		...
		if (bar->is_64bit) {
			...
			bar_high = pci_config_read32(
			    bus, device, function, bar_offset + 4);
			bar->phys_addr = ((uint64_t)bar_high << 32) |
			                 (bar_value & PCI_BAR_MEM_MASK);

			size_mask =
			    pci_bar_sizing(bus, device, function, bar_num);
			size_mask &= PCI_BAR_MEM_MASK;

			uint32_t orig_high = bar_high;
			pci_config_write32(
			    bus, device, function, bar_offset + 4, 0xFFFFFFFF);
			uint32_t size_high = pci_config_read32(
			    bus, device, function, bar_offset + 4);
			pci_config_write32(
			    bus, device, function, bar_offset + 4, orig_high);

			uint64_t size_mask_64 =
			    ((uint64_t)size_high << 32) | size_mask;
...
pci_map_bar(pci_device_t *dev, uint8_t bar_num)
{
	...
	flags = PAGING_FLAG_PRESENT | PAGING_FLAG_WRITE | PAGING_FLAG_NOCACHE |
	        PAGING_FLAG_GLOBAL;
	...
	if (!paging_map_range(kernel_space->page_dir,
	                      (uint64_t)bar->virt_addr,
	                      bar->phys_addr,
	                      num_pages,
	                      flags)) {
...
pci_unmap_bar(pci_device_t *dev, uint8_t bar_num)
{
	...
	paging_unmap_range(
	    kernel_space->page_dir, (uint64_t)bar->virt_addr, num_pages);
	vmm_free(kernel_space, bar->virt_addr, bar->size);
...
{
	uint16_t command =
	    pci_config_read16(bus, device, function, PCI_CONFIG_COMMAND);
...
{
	uint16_t status =
	    pci_config_read16(bus, device, function, PCI_CONFIG_STATUS);
	return (status & PCI_STATUS_CAP_LIST) != 0;
}

uint8_t
pci_find_capability(uint8_t bus,
                    uint8_t device,
                    uint8_t function,
                    uint8_t cap_id)
{
	...
	cap_ptr =
	    pci_config_read8(bus, device, function, PCI_CONFIG_CAPABILITIES);
	cap_ptr &= 0xFC;
...
pci_scan_capabilities(pci_device_t *dev)
{
	...
	if (!pci_has_capability_list(
	        dev->loc.bus, dev->loc.device, dev->loc.function))
		return;

	cap_ptr = pci_config_read8(dev->loc.bus,
	                           dev->loc.device,
	                           dev->loc.function,
	...
	while (cap_ptr != 0 && iterations < 48) {
		cap_id = pci_config_read8(
		    dev->loc.bus, dev->loc.device, dev->loc.function, cap_ptr);

		cap = kmalloc(sizeof(pci_capability_t));
		if (cap == NULL) {
			printf(
			    "PCI: Failed to allocate capability structure\n");
			break;
		}
...
pci_msi_init(pci_device_t *dev)
{
	...
	ctrl = pci_config_read16(dev->loc.bus,
	                         dev->loc.device,
	                         dev->loc.function,
...
pci_msi_enable(pci_device_t *dev, uint64_t address, uint16_t data)
{
	...
	pci_config_write32(dev->loc.bus,
	                   dev->loc.device,
	                   dev->loc.function,
	...
	if (dev->msi.is_64bit) {
		pci_config_write32(dev->loc.bus,
		                   ...
		                   (uint32_t)(address >> 32));
		pci_config_write16(dev->loc.bus,
		                   dev->loc.device,
		                   dev->loc.function,
...
pci_msix_init(pci_device_t *dev)
{
	...
	table_reg = pci_config_read32(dev->loc.bus,
	                              dev->loc.device,
	                              dev->loc.function,
	                              msix_cap->offset + PCI_MSIX_TABLE);
	pba_reg = pci_config_read32(dev->loc.bus,
	                            dev->loc.device,
	                            dev->loc.function,
	                            msix_cap->offset + PCI_MSIX_PBA);
	...
	dev->msix.table_virt =
	    (void *)((uint8_t *)table_bar->virt_addr + dev->msix.table_offset);
...
int
pci_msix_set_vector(pci_device_t *dev,
                    uint16_t vector,
                    uint64_t address,
                    uint32_t data)
{
...
pci_scan_function(uint8_t bus, uint8_t device, uint8_t function)
{
	...
	dev->id.subsys_vendor_id = pci_config_read16(
	    bus, device, function, PCI_CONFIG_SUBSYS_VENDOR_ID);
	dev->id.subsys_id =
	    pci_config_read16(bus, device, function, PCI_CONFIG_SUBSYS_ID);
	...
	dev->class.revision_id =
	    pci_config_read8(bus, device, function, PCI_CONFIG_REVISION_ID);

	/* Read header type */
	dev->header_type =
	    pci_get_header_type(bus, device, function) & PCI_HEADER_TYPE_MASK;
	dev->multifunction = (pci_get_header_type(bus, device, function) &
	                      PCI_HEADER_MULTIFUNCTION) != 0;

	/* Read interrupt information */
	dev->interrupt_line =
	    pci_config_read8(bus, device, function, PCI_CONFIG_INTERRUPT_LINE);
	dev->interrupt_pin =
	    pci_config_read8(bus, device, function, PCI_CONFIG_INTERRUPT_PIN);
...
pci_scan_bus_devices(uint8_t bus)
{
	...
	for (device = 0; device < PCI_MAX_DEVICE; device++) {
		...
		if (pci_is_multifunction(bus, device)) {
			for (function = 1; function < PCI_MAX_FUNCTION;
			     function++) {
...
pci_class_name(uint8_t class_code)
{
	...
	case PCI_CLASS_ACCELERATOR:
		return "Accelerator";
	case PCI_CLASS_INSTRUMENTATION:
...
pci_capability_name(uint8_t cap_id)
{
	...
	case PCI_CAP_ID_SHPC:
		return "Hot-Plug Controller";
	case PCI_CAP_ID_SSVID:
		return "Bridge Subsystem ID";
