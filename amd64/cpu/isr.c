...
void
isr_register_handler(uint8_t n, isr_handler_t handler)
{
	interrupt_handlers[n] = handler;
}

void
isr_unregister_handler(uint8_t n)
{
...
isr_install(void)
{
	idt_set_gate(T_DIVIDE,
	             (uint64_t)isr0,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_TRCTRAP,
	             (uint64_t)isr1,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_NMI,
	             ...
	             IDT_GATE_INTERRUPT,
	             IDT_IST_NMI);
	idt_set_gate(T_BPTFLT,
	             (uint64_t)isr3,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_USER_TRAP,
	             0);
	idt_set_gate(T_OFLOW,
	             (uint64_t)isr4,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_BOUND,
	             (uint64_t)isr5,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_PRIVINFLT,
	             (uint64_t)isr6,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_DNA,
	             ...
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_DOUBLEFLT,
	             (uint64_t)isr8,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             IDT_IST_DOUBLEFLT);
	idt_set_gate(T_FPOPFLT,
	             (uint64_t)isr9,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_TSSFLT,
	             (uint64_t)isr10,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_SEGNPFLT,
	             (uint64_t)isr11,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_STKFLT,
	             (uint64_t)isr12,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_PROTFLT,
	             (uint64_t)isr13,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_PAGEFLT,
	             (uint64_t)isr14,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_RESERVED,
	             (uint64_t)isr15,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_ARITHTRAP,
	             (uint64_t)isr16,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_ALIGNFLT,
	             (uint64_t)isr17,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	...
	idt_set_gate(T_IRQ9,
	             ...
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_IRQ10,
	             (uint64_t)irq10,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_IRQ11,
	             (uint64_t)irq11,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_IRQ12,
	             (uint64_t)irq12,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_IRQ13,
	             (uint64_t)irq13,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_IRQ14,
	             (uint64_t)irq14,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
	idt_set_gate(T_IRQ15,
	             (uint64_t)irq15,
	             GDT_SELECTOR_KERNEL_CODE,
	             IDT_GATE_INTERRUPT,
	             0);
