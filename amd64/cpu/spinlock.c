...
#endif
{
	...
	if (spinlock_holding(lock)) {
		panic_fmt("spinlock_acquire: recursive lock detected on '%s' by CPU %d",
		          spinlock_get_name(lock), get_cpu_id());
	}
...
bool spinlock_try_acquire(spinlock_t *lock) {
	...
	/* Try to acquire - if we get 0, we got the lock */
...
bool spinlock_holding(spinlock_t *lock) {
	...
	return atomic_load_int(&lock->locked) != 0 && 
	       lock->cpu == get_cpu_id();
}
