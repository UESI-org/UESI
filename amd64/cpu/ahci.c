...
ahci_init(void)
{
	...
	while (dev) {
		...
		if (pci_get_prog_if(dev->loc.bus,
		                    dev->loc.device,
		                    dev->loc.function) == AHCI_PCI_PROG_IF) {
...
ahci_controller_t *
ahci_probe_controller(pci_device_t *pci_dev)
{
	...
	pci_enable_bus_mastering(
	    pci_dev->loc.bus, pci_dev->loc.device, pci_dev->loc.function);
	pci_enable_memory_space(
	    pci_dev->loc.bus, pci_dev->loc.device, pci_dev->loc.function);

	/* Get ABAR (BAR5) - AHCI Base Address */
	if (pci_read_bar(pci_dev->loc.bus,
	                 pci_dev->loc.device,
	                 pci_dev->loc.function,
	                 5,
	                 &pci_dev->bars[5]) < 0) {
	...
	ctrl->num_cmd_slots =
	    ((ctrl->cap & AHCI_CAP_NCS) >> AHCI_CAP_NCS_SHIFT) + 1;
	...
	for (i = 0; i < AHCI_MAX_PORTS; i++) {
		if (pi & (1 << i)) {
			...
			ctrl->ports[i].regs =
			    (volatile ahci_port_regs_t *)(ctrl->abar + 0x100 +
			                                  (i * 0x80));

			if (ahci_port_init(ctrl, i) == 0) {
				ahci_device_type_t type =
				    ahci_port_probe(&ctrl->ports[i]);
				ctrl->ports[i].device_type = type;

				if (type != AHCI_DEV_NULL) {
					printf("AHCI: Port %u: %s device "
					       "detected\n",
					       i,
					       ahci_device_type_string(type));
				}
			}
		}
	}
...
void
ahci_enable_ahci(ahci_controller_t *ctrl)
{
	uint32_t ghc;
...
int
ahci_reset_controller(ahci_controller_t *ctrl)
{
	uint32_t timeout;
...
int
ahci_port_init(ahci_controller_t *ctrl, uint8_t port_num)
{
	ahci_port_t *port;
...
int
ahci_port_rebase(ahci_port_t *port)
{
	uint32_t i;
	uint64_t phys;
	void *virt;
...
int
ahci_port_start(ahci_port_t *port)
{
	uint32_t cmd;
...
int
ahci_port_stop(ahci_port_t *port)
{
	uint32_t cmd;
	uint32_t timeout;
...
ahci_device_type_t
ahci_port_probe(ahci_port_t *port)
{
	uint32_t ssts, sig;
...
void
ahci_port_clear_errors(ahci_port_t *port)
{
	if (!port || !port->regs)
		return;
...
int
ahci_port_wait_ready(ahci_port_t *port, uint32_t timeout_ms)
{
	uint32_t tfd;
...
int
ahci_port_reset(ahci_port_t *port)
{
	uint32_t sctl, ssts;
	uint32_t timeout;
...
int
ahci_port_find_cmdslot(ahci_port_t *port)
{
	uint32_t slots;
	uint32_t i;
...
int
ahci_port_read(ahci_port_t *port, uint64_t lba, uint32_t count, void *buf)
{
	int slot;
	...
	/* Setup command header */
	cmd_hdr->cfl =
	    sizeof(ahci_fis_reg_h2d_t) / 4; /* FIS length in DWORDS */
...
int
ahci_port_write(ahci_port_t *port,
                uint64_t lba,
                uint32_t count,
                const void *buf)
{
...
int
ahci_port_identify(ahci_port_t *port, void *buf)
{
	int slot;
	ahci_cmd_header_t *cmd_hdr;
	ahci_cmd_table_t *cmd_tbl;
	...
	cmd = (port->device_type == AHCI_DEV_SATAPI) ? ATA_CMD_IDENTIFY_PACKET
	                                             : ATA_CMD_IDENTIFY;
...
void
ahci_interrupt_handler(void *data)
{
	ahci_controller_t *ctrl;
	uint32_t is, port_is;
	uint8_t i;
...
const char *
ahci_device_type_string(ahci_device_type_t type)
{
	switch (type) {
	case AHCI_DEV_SATA:
		return "SATA";
...
void
ahci_dump_controller_info(ahci_controller_t *ctrl)
{
	if (!ctrl)
		return;
...
void
ahci_dump_port_info(ahci_port_t *port)
{
